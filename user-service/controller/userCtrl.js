    ...
    try {
        ...
        if (!findUser) {
            res.status(401).json({
              ...
              message: 'The provided email address is not associated with any user account. Please check your email or sign up for a new account.',
            });
            return;
        }else{
...
export const createAddress=async(req,res)=>{
    try {
        ...
        const {fullName,mobile,region,pinCode,areaStreet,ladmark,townCity,state,adressType}=req.body
...
export const editAddress=async(req,res)=>{
    try {
        const {fullName,mobile,region,pinCode,areaStreet,ladmark,townCity,state,adressType,id}=req.body
        const userCr=req.user
    ...
    try {
        ...
        const transporter = nodemailer.createTransport({
          ...
          if (info) {
            ...
            console.log("FORGET otp ende %s", info);
      
            res.status(200).json("Check your mail and verify ...");
          }

    } catch (error) {
        res.status(400).json("Error in User-service  requestForchangePassword  " + error);
...
export const verifyotpChagePAssword=async(req,res)=>{
    try {
        ...
        res.status(400).json("Error in User-service  verifyotpChagePAssword  " + error);
...
export const createOrder=async(req,res)=>{
    try {
        ...
        res.status(200).json('Order Created success full take your orders to see the order..!')
